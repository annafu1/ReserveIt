  ...
  return (
    <div>
      <h2 id="restaurant-heading" data-cy="RestaurantHeading">
        <Translate contentKey="reserveItApp.restaurant.home.title">Restaurants</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="reserveItApp.restaurant.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/restaurant/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="reserveItApp.restaurant.home.createLabel">Create new Restaurant</Translate>
          </Link>
        </div>
      </h2>
        ...
        {restaurantList && restaurantList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="reserveItApp.restaurant.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {restaurantList.map((restaurant, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`reserveItApp.CuisineType.${restaurant.cuisine}`} />
                  </td>
                  <td>{restaurant.streetAddress}</td>
                  <td>{restaurant.postalCode}</td>
                  <td>{restaurant.city}</td>
                  <td>{restaurant.state}</td>
                  <td>{restaurant.foodMenu ? <Link to={`/food-menu/${restaurant.foodMenu.id}`}>{restaurant.foodMenu.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="reserveItApp.restaurant.home.notFound">No Restaurants found</Translate>
            </div>
          )
        )}
      </div>
